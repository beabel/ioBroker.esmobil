...
class ESmobil extends utils.Adapter {
    ...
    async poll() {
        ...
        await this.ensureState('info.connection', { name: 'Verbunden mit den Schulservern', type: 'boolean', role: 'indicator.connected', read: true, write: false, def: false }, connected);
    }
    ...
    async main() {
        ...
        if (config.pollHomeworkEtc) {
            if (config.haUsername) {
                ...
                this.log.warn('Kein Home.InfoPoint-Benutzername eingetragen - überspringe Hausaufgaben/Bemerkungen/Zensuren.');
            }
        }
    ...
    async updateTimetable(school, config) {
        ...
        await this.ensureState(
            'plan.week.days',
            { name: 'Wochenplan - alle Tage in einem JSON-Array', type: 'string', role: 'json', read: true, write: false, def: '[]' },
            JSON.stringify(weekDays)
        );
    }

    async writeDayPlan(prefix, dayPlan) {
        await this.ensureState(`${prefix}.date`, { name: 'Datum', type: 'string', role: 'date', read: true, write: false, def: '' }, isoDateOf(dayPlan.dateKey));
        await this.ensureState(`${prefix}.sourceTimestamp`, { name: 'Stand der Daten (Server)', type: 'string', role: 'text', read: true, write: false, def: '' }, dayPlan.sourceTimestamp || '');
        await this.ensureState(`${prefix}.lessonCount`, { name: 'Anzahl Stunden', type: 'number', role: 'value', read: true, write: false, def: 0 }, dayPlan.lessons.length);
        await this.ensureState(`${prefix}.lessons`, { name: 'Stunden (JSON)', type: 'string', role: 'json', read: true, write: false, def: '[]' }, JSON.stringify(dayPlan.lessons));
    }
    ...
    async updateHomeInfoPoint(school, config) {
        ...
        await this.ensureState('homework.count', { name: 'Anzahl Hausaufgaben', type: 'number', role: 'value', read: true, write: false, def: 0 }, homework.length);
        await this.ensureState('homework.entries', { name: 'Hausaufgaben (JSON)', type: 'string', role: 'json', read: true, write: false, def: '[]' }, JSON.stringify(homework));
        await this.ensureState('homework.newCount', { name: 'Neue Hausaufgaben seit dem letzten Abruf', type: 'number', role: 'value', read: true, write: false, def: 0 }, newHomework.length);
        await this.ensureState('homework.newEntries', { name: 'Neue Hausaufgaben seit dem letzten Abruf (JSON)', type: 'string', role: 'json', read: true, write: false, def: '[]' }, JSON.stringify(newHomework));

        const remarks = parseRemarks(html);
        const newRemarks = await this.detectNew('remarks', remarks, (r) => `${r.date}|${r.type}|${r.subject}|${r.text}`);
        await this.ensureState('remarks.count', { name: 'Anzahl Bemerkungen', type: 'number', role: 'value', read: true, write: false, def: 0 }, remarks.length);
        await this.ensureState('remarks.entries', { name: 'Bemerkungen (JSON)', type: 'string', role: 'json', read: true, write: false, def: '[]' }, JSON.stringify(remarks));
        await this.ensureState('remarks.newCount', { name: 'Neue Bemerkungen seit dem letzten Abruf', type: 'number', role: 'value', read: true, write: false, def: 0 }, newRemarks.length);
        await this.ensureState('remarks.newEntries', { name: 'Neue Bemerkungen seit dem letzten Abruf (JSON)', type: 'string', role: 'json', read: true, write: false, def: '[]' }, JSON.stringify(newRemarks));
        ...
        await this.ensureState('grades.subjectCount', { name: 'Anzahl Fächer mit mindestens einer Zensur', type: 'number', role: 'value', read: true, write: false, def: 0 }, subjectsWithGrades.length);
        await this.ensureState('grades.bySubject', { name: 'Alle Fächer als JSON (auch ohne Zensuren)', type: 'string', role: 'json', read: true, write: false, def: '{}' }, JSON.stringify(gradesObj));
        ...
        for (const [label, entries] of subjectsWithGrades) {
            ...
            await this.ensureState(`${prefix}.label`, { name: 'Fach', type: 'string', role: 'text', read: true, write: false, def: '' }, label);
            await this.ensureState(`${prefix}.count`, { name: 'Anzahl Zensuren', type: 'number', role: 'value', read: true, write: false, def: 0 }, entries.length);
            await this.ensureState(`${prefix}.average`, { name: 'Durchschnitt als Zahl (nur numerisch auswertbare Zensuren)', type: 'number', role: 'value', read: true, write: false, def: 0 }, rawAverage !== null ? roundTo2(rawAverage) : null);
            await this.ensureState(`${prefix}.averageNote`, { name: 'Durchschnitt als Zensur (wie im PHP-Original, z. B. "1+")', type: 'string', role: 'text', read: true, write: false, def: '-' }, gradeAverageLabel(rawAverage));
            await this.ensureState(`${prefix}.entries`, { name: 'Zensuren (JSON)', type: 'string', role: 'json', read: true, write: false, def: '[]' }, JSON.stringify(entries));
        }
        ...
        await this.ensureState(
            ...
            { name: 'Durchschnitt über alle Fächer als Zahl (unwertet je Einzelnote, nicht je Fach)', type: 'number', role: 'value', read: true, write: false, def: 0 },
            rawOverallAverage !== null ? roundTo2(rawOverallAverage) : null
        );
        await this.ensureState(
            'grades.overallAverageNote',
            { name: 'Durchschnitt über alle Fächer als Zensur', type: 'string', role: 'text', read: true, write: false, def: '-' },
        ...
        const newGrades = await this.detectNew('grades', flatGrades, (g) => `${g.subject}|${g.date}|${g.grade}|${g.remark}`);
        await this.ensureState('grades.newCount', { name: 'Neue Zensuren seit dem letzten Abruf', type: 'number', role: 'value', read: true, write: false, def: 0 }, newGrades.length);
        await this.ensureState('grades.newEntries', { name: 'Neue Zensuren seit dem letzten Abruf (JSON)', type: 'string', role: 'json', read: true, write: false, def: '[]' }, JSON.stringify(newGrades));

        const totalNew = newHomework.length + newRemarks.length + newGrades.length;
        await this.ensureState('info.newItemsCount', { name: 'Neue Einträge in diesem Abruf (Hausaufgaben+Bemerkungen+Zensuren)', type: 'number', role: 'value', read: true, write: false, def: 0 }, totalNew);
        if (totalNew > 0) {
            ...
            await this.ensureState('info.lastNewAt', { name: 'Zeitpunkt des letzten Neuzugangs', type: 'string', role: 'value.time', read: true, write: false, def: '' }, new Date().toISOString());
            this.log.info(`Neu seit dem letzten Abruf: ${newHomework.length} Hausaufgabe(n), ${newRemarks.length} Bemerkung(en), ${newGrades.length} Zensur(en).`);
        }
    }
...
if (require.main !== module) {
    ...
    module.exports.mondayOfRelevantWeek = mondayOfRelevantWeek;
    module.exports.formatDateKey = formatDateKey;
    module.exports.isoDateOf = isoDateOf;
    module.exports.slugifySubject = slugifySubject;
    module.exports.parseGradeValue = parseGradeValue;
    module.exports.gradeAverageLabel = gradeAverageLabel;
    module.exports.mean = mean;
    module.exports.roundTo2 = roundTo2;
} else {
    new ESmobil();
}
