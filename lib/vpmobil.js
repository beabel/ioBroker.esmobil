 ...
 *   VpMobil > ZusatzInfo > ZiZeile                          (allgemeine Hinweiszeile(n) für den Tag, unabhängig von der Klasse)
 */
...
const xmlParser = new XMLParser({
    ...
    isArray: name => ['Kl', 'Std', 'Ku', 'Ue', 'ZiZeile'].includes(name),
});

/**
 * Normalisiert einen fast-xml-parser-Knoten (String | {"#text": string, ...} | undefined) zu einem getrimmten String.
 *
 * @param node fast-xml-parser node value to normalize
 */
...
/**
 * HTTP client for the VpMobil/Indiware mobdaten interface (basic auth + fixed file paths).
 */
class VpMobilClient {
    /**
     ...
     * @param {string} username VpMobil username
     * @param {string} password VpMobil password
     */
     ...
     *
     * @param fileName mobdaten file name, e.g. "Klassen.xml" or "PlanKl20260904.xml"
     */
    ...
    async fetchDirectoryListing() {
        try {
            ...
            const response = await axios.post(`${this.baseUrl}/_phpmob/vpdir.php`, params, {
                auth: { username: this.username, password: this.password },
            ...
            const code = err.response ? err.response.status : undefined;
            throw new VpMobilError(`HTTP ${code || err.message} beim Laden der Dateiliste`, code);
        }
    }
}
 ...
 *
 * @param fileNames file names as listed by fetchDirectoryListing()
 */
 ...
 *
 * @param xml raw content of Klassen.xml
 */
 ...
 *
 * @param xml raw content of a PlanKlYYYYMMDD.xml file
 * @param dateKey date key (yyyyMMdd) this file was fetched for
 * @param klasse class name to extract lessons for, e.g. "08m2"
 * @returns {{dateKey: string, lessons: Array<object>, sourceTimestamp: string|null, zusatzInfo: Array<string>}} parsed day plan
 */
function parseDayPlan(xml, dateKey, klasse) {
    const doc = xmlParser.parse(xml);
    const root = doc?.VpMobil || {};
    const sourceTimestamp = textOf(root?.Kopf?.zeitstempel) || null;
    const zusatzInfo = asArray(root?.ZusatzInfo?.ZiZeile)
        .map(textOf)
        .filter(v => v.length > 0);
    ...
    if (target) {
        ...
        for (const std of stdList) {
            ...
            const faChanged = !!(faNode && typeof faNode === 'object' && faNode['@_FaAe'] !== undefined);
    ...
    return { dateKey, lessons, sourceTimestamp, zusatzInfo };
}
