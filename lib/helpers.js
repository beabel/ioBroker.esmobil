...
const WEEKDAY_NAMES_EN = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'];

/**
 * Vollständige 11-Sprachen-Namen für die plan.day1-5-Kanäle (nur en/de fließen zusätzlich
 * in den JSON-Payload von plan.week.days ein, siehe WEEKDAY_NAMES/WEEKDAY_NAMES_EN oben -
 * common.name der Kanäle braucht dagegen alle 11 Sprachen wie jeder andere Objektname auch).
 */
const WEEKDAY_NAMES_I18N = [
    {
        en: 'Monday',
        de: 'Montag',
        ru: 'Понедельник',
 ...
 * @returns {string} date as "yyyyMMdd", matching VpMobil's file naming convention
 */
 ...
 *
 * @param dateKey date key in "yyyyMMdd" format, or empty string if unavailable
 */
 ...
 *
 * @param date reference date to find the relevant Monday for
 */
 ...
 *
 * @param label subject label as shown by Home.InfoPoint, e.g. "DE - Deutsch (Schuster)"
 */
 ...
 *
 * @param grade raw grade text as read from Home.InfoPoint, e.g. "2+" or "e.n."
 */
 ...
 */
// The @type below is needed despite checkJs: without it, TS infers (string|number)[][]
// instead of tuples, breaking `Math.abs(v - w)` below.
 ...
 *
 * @param value grade average as a number, or null if none could be computed
 */
...
/**
 * @param {number[]} numbers values to average
 * @returns {number} arithmetic mean of the given numbers
 */
 ...
 * @returns {number} n rounded to 2 decimal places
 */
function roundTo2(n) {
