...
/**
 * "yyyyMMdd" (all-day) oder "yyyyMMdd'T'HHmmss['Z']" -> "dd.MM.yyyy", passend zum
 * Datumsformat der Home.InfoPoint-Hausaufgaben, damit beide Quellen in derselben Liste
 * landen. Ein reines Datum (all-day) oder eine "floating" Zeit ohne "Z" wird unverändert
 * als Ortszeit behandelt (RFC 5545) - nur ein echter UTC-Zeitstempel ("Z"-Suffix) wird VOR
 * dem Formatieren nach Europe/Berlin umgerechnet. Ohne diese Umrechnung würde z. B. eine um
 * Mitternacht Ortszeit fällige Aufgabe (00:00 CEST = 22:00 UTC am Vortag) fälschlich einen
 * Tag zu früh angezeigt.
 *
...
function formatIcsDate(raw) {
    ...
    const [, y, mo, d, hh, mm, ss, isUtc] = m;
    if (hh !== undefined && isUtc) {
        const utcInstant = new Date(Date.UTC(Number(y), Number(mo) - 1, Number(d), Number(hh), Number(mm), Number(ss)));
        return berlinDateFormat.format(utcInstant);
    }
    return `${d}.${mo}.${y}`;
}
 ...
 *
 * @param {string} ics rohes iCalendar-Dokument
 * @returns {Array<{date: string, subject: string, task: string, source: string, description?: string}>} parsed calendar entries
 */
